...
Defense layers (in order):
  L1 — per-chat token bucket (1 msg/sec, burst 4)
  ...
  L4 — reactive lockout on RetryAfter (2N wait, no retries during)
  L5 — edit coalesce per message_id (max 8 edits per message)
  L6 — Bot._do_post monkey-patch (belt-and-suspenders for any code
       path that bypasses TelegramAdapter)

Designed to NEVER let a Telegram-flood-block recur. Drops at the gateway
rather than at Telegram — strikes never accumulate.
"""
...
class TelegramRateLimiter:
    ...
    async def acquire_send(self, chat_id, text: Optional[str] = None) -> bool:
        ...
        async with self._lock:
            ...
            if lock_until > now:
                ...
                logger.debug("[ratelimit] L4 chat=%s locked for %.1fs more",
                             chat_key, lock_until - now)
                return False
            ...
            if text:
                ...
                if now - last < self.DEDUP_WINDOW_SEC:
                    self._dropped_count += 1
                    logger.debug("[ratelimit] L2 dedup chat=%s text=%r",
                                 chat_key, text[:80])
                    return False

            # L3 global
            while self._global_window and now - self._global_window[0] >= self.GLOBAL_WINDOW:
                self._global_window.popleft()
            if len(self._global_window) >= self.GLOBAL_RATE:
                self._dropped_count += 1
                logger.debug("[ratelimit] L3 global cap chat=%s window=%d",
                             chat_key, len(self._global_window))
                return False

            # L1 per-chat token bucket
            tokens, last_refill = self._chat_buckets.get(
                chat_key, (self.PER_CHAT_BURST, now)
            )
            tokens = min(self.PER_CHAT_BURST,
                         tokens + (now - last_refill) * self.PER_CHAT_RATE)
            if tokens < 1.0:
                self._dropped_count += 1
                logger.debug("[ratelimit] L1 bucket empty chat=%s tokens=%.2f",
                             chat_key, tokens)
                return False
            tokens -= 1.0

            # commit
            self._chat_buckets[chat_key] = (tokens, now)
    ...
    async def acquire_edit(self, chat_id, message_id) -> bool:
        ...
        async with self._lock:
            ...
            if count >= self.MAX_EDITS_PER_MESSAGE:
                self._dropped_count += 1
                logger.debug("[ratelimit] L5 edit cap chat=%s msg=%s count=%d",
        ...
        async with self._lock:
            ...
            if until > cur:
                ...
                logger.warning(
                    ...
                    "locking chat=%s for %.1fs (2x backoff)",
                    retry_after_sec, chat_key, until - now,
                )

    def reset_edit_count(self, chat_id, message_id):
        self._edit_counts.pop((str(chat_id), str(message_id)), None)
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "chats_locked_now": sum(1 for t in self._chat_locks.values() if t > now),
        }
